...
let referralSystem = {
    init() {
        ...
        if (window.Telegram && window.Telegram.WebApp) {
            ...
            if (webApp.initDataUnsafe && webApp.initDataUnsafe.user) {
                ...
                const botUsername = webApp.initDataUnsafe.bot?.username || 'your_bot';
    ...
    updateReferralStats() {
        if (window.Telegram && window.Telegram.WebApp && window.Telegram.WebApp.initDataUnsafe.user) {
            // Здесь будет запрос к серверу для получения актуальной статистики
            // Пока используем моковые данные
            this.referralCount.textContent = '0';
            this.referralBonus.textContent = '0 USDT';
